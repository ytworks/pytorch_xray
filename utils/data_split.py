...
def data_split(ini, labels, debug_mode):
    ...
    tv_list = [x[0] for x in pd.read_csv(
        ini.get('data', 'train_path'), usecols=[0]).values]
    test_list = [x[0] for x in pd.read_csv(
        ini.get('data', 'test_path'), usecols=[0]).values]
    ...
    if debug_mode:
        train_list = train_list[0:32]
        val_list = val_list[0:32]
        test_list = test_list[0:32]
